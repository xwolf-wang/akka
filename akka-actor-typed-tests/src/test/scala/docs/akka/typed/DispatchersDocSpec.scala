...
class DispatchersDocSpec extends ScalaTestWithActorTestKit(DispatchersDocSpec.config) with WordSpecLike {
  ...
  "Actor Dispatchers" should {
    "support default and blocking dispatcher" in {
      ...
      probe.receiveOne().id shouldEqual "akka.actor.default-dispatcher"
      ...
      probe.receiveOne().id shouldEqual "akka.actor.default-blocking-io-dispatcher"
      ...
      probe.receiveOne().id shouldEqual "your-dispatcher"
    }
  }
}
